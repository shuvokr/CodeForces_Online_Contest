...
struct Solution
{
    ...
    int coinChange(vector<int>& coins, int amount) 
	{
    	...
    	return res = DP(0, amount) > 1000000 ? -1 : DP(0, amount);
    }
    
    int DP(int pos, int amount)
    {
    	phl;
    	if(amount == 0) return 0;
    	if(pos == v.size()) return 1000001;
		int &ret = mamo[ pos ][ amount ];
    	if(ret != -1) return ret;
    	ret = INF;
    	
    	ret = min(ret, DP(pos + 1, amount));
    	int x = 1;
    	while(amount - x * v[ pos ] >= 0) ret = min(ret, x + DP(pos + 1, amount - x * v[ pos ]));
    	return ret;
	}
};
...
{
	vector <int> vv;
	vv.push_back( 1 ); vv.push_back( 2 ); vv.push_back( 5 );
	Solution ans; 
	ans.coinChange(vv, 11);
    cout << ans.res << endl;
	//#ifdef monkey
