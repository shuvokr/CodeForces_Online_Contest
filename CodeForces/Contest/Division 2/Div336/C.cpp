...
{
	si1( n );
	for(int i = 0; i < n; i++) si2(in[ i ].ai, in[ i ].bi);
	sort(in, in + n, cmp);
	reverse(in, in + n);
	CLR(mamo, -1);
	cout << DP(0, 0) << endl;
...
int DP(int pos, int flag)
{
	...
	if(flag == 0) ret = 1 + DP(pos + 1, 0);
	
	int a, b, x = in[pos].ai - 1, y = in[pos].ai - in[pos].bi;
	
	int low = pos + 1, high = n - 1, mid;
	while(high - low > 1)
	{
		mid = (low + high) / 2;
		if(in[mid].ai <= x) high = mid;
		else low = mid; 
	}
	if(in[low].ai <= x) a = low;
	else a = high;
	
	if(in[a].ai > x) a++;
	
	low = pos + 1, high = n - 1, mid;
	while(high - low > 1)
	{
		mid = (low + high) / 2;
		if(in[mid].ai >= y) low = mid;
		else high = mid; 
	}
	if(in[high].ai >= y) b = high;
	else b = low;
	...
	*/
	...
	int tot = b - a + 1, next = b + 1;
	if(a > b) tot = 0, next = pos + 1;
	
	//pi3(pos, a, b);
	ret = min(ret, tot + DP(next, 1));
