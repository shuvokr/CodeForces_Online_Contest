...
void input()
{
    long long n, r, avr, sum = 0;
    cin >> n >> r >> avr;
    map <long long, long long int> mp, p;
    mp.clear();
    for(long long i = 0; i < n; i++)
    {
        cin >> in[i].a >> in[i].b;
        sum += in[i].a;
        mp[ in[i].a ] = max(mp[ in[i].a ], in[i].b);
    }
    sort(in, in + n, cmp);

    long long need = (n * avr) - sum;
    //cout << "  " << need << endl;
    if(need <= 0)
    {
        puts("0");
    ...
    {
        long long x = in[i].a, y = in[i].b, tot = y;
        while(need && mp[ x ] >= tot && x < r)
        {
            //cout << mp[ x ] << " " << x << " " << y << endl;
            need--; tot++;
            res += y;
