...
lld DP(lld tim, lld val, lld flag, lld s);

lld mamo[ 105 ][ 2110 ][ 11 ][ 2 ], v1, v2, tottim, d;
...
lld DP(lld tim, lld val, lld flag, lld s)
{
    ...
    lld x = 1005 + val;
    lld &ret = mamo[ tim ][ x ][ flag ][ s ];
    if(ret != -1) return ret;
    ret = -10000000000000;
    for(int i = 0; i <= d; i++)
    {
        lld a = val + DP(tim - 1, val - i, i, 0);
        lld b = val + DP(tim - 1, val + i, i, 1);
        ret = max(ret, max(a, b));
    }
    return ret;
}
