...
void input()
{
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        waight += w; in[i].ind = i;
        in[i].wid = w;
        inn[ i ].hight = in[ i ].hight;
        inn[ i ].ind = in[ i ].ind;
        inn[ i ].wid = in[ i ].wid;
    }

    sort(in, in + n, cmp);
    int a = in[0].hight, b = in[1].hight;

    if(inn[0].hight == a) pf("%d", b * (waight - inn[0].wid));
    else pf("%d", a * (waight - inn[0].wid));

    for(int i = 1; i < n; i++)
    {
        if(inn[i].hight == a) pf(" %d", b * (waight - inn[i].wid));
        else pf(" %d", a * (waight - inn[i].wid));
    }
    puts("");
}
