...
{
    lld sum;
    sf("%I64d %I64d", &n, &s); sum = s;
    for(int i = 0; i < n; i++)
    {
        sf("%I64d %I64d %I64d", &in[i].x, &in[i].y, &in[i].a);
        sum += in[i].a; in[i].len = sq(in[i].x) + sq(in[i].y);
    }
    if(sum < 1000000)
    {
        puts("-1");
    ...
    for(int i = 0; i < n; i++)
    {
        lld tot = s;
        for(int j = 0; j < n; j++)
            if(in[j].len <= in[i].len) tot += in[j].a;
        if(tot >= 1000000 && ans > in[i].len) ans = in[i].len;
    }
    pf("%.8lf\n", sqrt((double)ans));
}
