    ...
    for(int i = 1; i <= n; i++)
    {
        ...
        else if(pos[ i ] > in[ i ])
        {
            ...
            if(i < n)
            {
                pos[i + 1] = (pos[ i ] - in[ i ]);
                pos[ i ] = in[ i ];
                pos[ i ] += pos[i - 1]; pos[i - 1] = 0;
            }
        }
        else
        {
            ans++; in[ i ] -= pos[ i ]; pos[ i ] += pos[i - 1]; pos[i - 1] = 0;
            if(pos[ i ] >= in[ i ])
            {
                ans++;
                if(i < n)
