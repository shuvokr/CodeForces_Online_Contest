...
{
    sf("%d %d", &n, &m);
    for(int i = 0; i < n; i++) {sf("%d", &in[ i ].first); in[ i ].second = i;}
    for(int i = 0; i < m; i++)
    {
        sf("%d %d", &s[i].l, &s[i].r);
        s[i].ln = s[i].r - s[i].l + 1;
    }

    sort(in, in + n);
    for(int i = 0; i < n; i++)
        if(i % 2) color[in[i].second] = 1;
        else color[in[i].second] = 0;

    for(int i = 0; i < n; i++) pf("%d ", color[ i ]);
}
