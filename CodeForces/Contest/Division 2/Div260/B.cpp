...
void input()
{
    ...
    {
        int two = 0, three = 0, four = 0, len = strlen( p );
        for(int i = 0; i < len; i++)
        {
            two = (two * 10 + (p[ i ] - 48)) % 4;
            three = (three * 10 + (p[ i ] - 48)) % 4;
            four = (four * 10 + (p[ i ] - 48)) % 2;
        }

        two = two == 0 ? 6 : two == 3 ? 8 : two == 2 ? 4 : 2;
        three = three == 0 ? 1 : three == 3 ? 7 : three == 2 ? 9 : 3;
        four = four == 0 ? 6 : 4;

        cout << ((1 + two + three + four) % 10) % 5 << endl;
    }
}
