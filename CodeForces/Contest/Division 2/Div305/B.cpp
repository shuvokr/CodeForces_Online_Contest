...
int in[ 510 ][ 510 ], tmp[ 510 ][ 510 ], r, c, q;
int ar[ 510 ];
...
void input()
{
    ...
    for(int i = 1; i <= r; i++)
    {
        for(int j = 1; j <= c; j++) si1(in[ i ][ j ]), tmp[ i ][ j ] = in[ i ][ j ];
        for(int j = 1; j <= c; j++)
        {
            if(in[ i ][ j ] == 0) continue;
            int k = j, cou = 0;
            while(in[ i ][ k ] == 1 && k <= c) cou++, k++;
            k--;
            for(int s = j; s <= k; s++) in[ i ][ s ] = cou;
            ar[ i ] = max(ar[ i ], cou);
            if(k > j) j = k;
        }
    }
...
//    }
    ...
    while( q-- )
    {
        ...
        tmp[ x ][ y ] = tmp[ x ][ y ] == 1 ? 0 : 1;
        for(int i = 1; i <= c; i++) in[ x ][ i ] = tmp[ x ][ i ];
        ar[ x ] = 0;
        for(int j = 1; j <= c; j++)
        {
            if(in[ x ][ j ] == 0) continue;
            int k = j, cou = 0;
            while(in[ x ][ k ] == 1 && k <= c) cou++, k++;
            k--;
            for(int s = j; s <= k; s++) in[ x ][ s ] = cou;
            ar[ x ] = max(ar[ x ], cou);
            if(k > j) j = k;
        }
        int ans = 0;
        for(int i = 1; i <= r; i++) ans = max(ans, ar[ i ]);
        pf("%d\n", ans);
    }
