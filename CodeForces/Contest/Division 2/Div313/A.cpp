...
{
    cin >> k >> in;
    lld i = 0, len = strlen( in ), tmp = 0, ress = 0;
    ...
    if(k == 0)
    {
        ...
        while(i < len)
        {
            if(in[ i ] == '1') { i++; for(int ii = 1; ii <= tmp; ii++) ress += ii; tmp = 0; continue; }
            tmp++; i++;
        }
        if(in[len - 1] == '0') { for(int ii = 1; ii <= tmp; ii++) ress += ii; }
        cout << ress << endl;
        return;
    }
    ...
    {
        a = i;
        while(in[ i ] == '0' && i < len) i++; l = i; tmp = k;
        while(i < len && tmp) tmp -= in[ i ] == '1' ? 1 : 0, i++; r = i - 1;
        while(in[ i ] == '0' && i < len) i++; b = i - 1;
        if(tmp) continue;

        lld x = l - a, y = 0, z = b - r; x++; z++; y++;
        //pf("%lld %lld %lld\n", x, y, z);
