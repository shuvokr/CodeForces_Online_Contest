...
double DP(int num, int tim);

double stor[ 2010 ][ 2010 ], p;
int mamo[ 2010 ][ 2010 ], n, t;
...
double DP(int num, int tim)
{
    if(tim == t)
    {
        return (double) min(n, num);
    }

    double &ret = stor[ num ][ tim ];
    if(mamo[ num ][ tim ] != -1) return ret;
    ret = 0.0;
    ret = (DP(num + 1, tim + 1) * p) + (DP(num, tim + 1) * (1.0 - p));
