...
bool cmpL(const data &x, const data &y)
{
    if(x.L < y.L) return true;
    else if(x.L == y.L && x.D > y.D) return true;
    return false;
}

int n, len[ 100010 ], di[ 100010 ];
int sum[ 100010 ];
...
{
    si1( n );
    for(int i = 0; i < n; i++) si1(len[ i ]);
    for(int i = 0; i < n; i++) si1(di[ i ]);
    ...
    for(int i = 1; i < n; i++) sum[ i ] = sum[i - 1] + ina[i].D;

    int i = 0;
    while( i < n )
    {
        int tt = ina[i].L;
        while(ina[i].L == tt && i < n) i++;
        track.pb(i - 1);
    }
