...
void input()
{
    lld edge = 0;
    sf("%I64d %I64d", &n, &m);
    for(int i = 0, t; i < n; i++) sf("%d", &t), in[ i + 1 ] = t;
    for(int i = 0, a, b; i < m; i++)
    {
        sf("%d %d", &a, &b);
        graph[ i ].x = a; graph[ i ].y = b;
        graph[ i ].cost = min(in[ a ], in[ b ]);
    }
    sort(graph, graph + m, cmp);
    for(int i = 0; i < m; i++)
    {
        if(mark[ graph[i].x ] && mark[ graph[i].y ]) continue;
        if(!mark[ graph[i].x ]) res.pb( in[ graph[i].x ] );
        if(!mark[ graph[i].y ]) res.pb( in[ graph[i].y ] );
        mark[ graph[i].x ] = mark[ graph[i].y ] = true;
        edge++;
    }
    sort(res.begin(), res.end());
    //pf("%lld\n", edge);
    edge++;
    lld len = res.size(); sum = 0;
    for(int i = 0; i < len; i++)
    {
        lld k = edge - i - 1;
        sum += k * res[ i ];
    }
    double ans = (double)(sum * 2) / double(n*(n - 1));
    pf("%.6lf\n", ans);
}
