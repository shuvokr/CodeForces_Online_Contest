...
void input()
{
    int sum = 0, n, in[ 5 ];
    cin >> n;
    for(int i = 0; i < n; i++) scanf("%d", &in[ i ]), sum += in[ i ];
    sort(in, in + n);
    if(n == 0) puts("YES\n1\n1\n3\n3");
    else if(n == 1)
    {
    ...
    {
        if(in[ 0 ] == in[ 1 ]) pf("YES\n%d\n%d\n", in[ 0 ] * 3, in[ 0 ] * 3);
        else if(in[ 0 ] * 3 == in[ 1 ]) pf("YES\n%d\n%d\n", in[ 0 ], in[ 0 ] * 3);
        else puts("NO");
    }
    else if(n == 3)
    {
        if(in[ 0 ] == in[ 1 ] && in[ 0 ] * 3 == in[ 2 ]) pf("YES\n%d\n", in[ 2 ]);
        else puts("NO");
    }
    else
    {
        if(sum % 4 || (in[ 1 ] + in[ 2 ]) % 2) puts("NO");
        else if(sum / 4 == (in[ 1 ] + in[ 2 ]) / 2 && sum / 4 == in[ 3 ] - in[ 0 ]) puts("YES");
        else puts("NO");
    }
}
