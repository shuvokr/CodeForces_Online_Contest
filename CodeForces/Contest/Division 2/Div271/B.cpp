...
{
    cin >> n;
    for(int i = 0; i < n; i++) sf("%d", &in[ i ]); sum[ 0 ] = in[ 0 ];
    for(int i = 1; i < n; i++) sum[ i ] = in[ i ] + sum[i - 1];
    int fst = 0, cou = 0;
    for(int i = 0; i < n; i++)
    {
        fst++;
    ...
    for(int ii = 0; ii < Q; ii++)
    {
        ...
        while(true)
        {
            //pf("%d %d\n", left, right);
            mid = (left + right) >> 1;
            if(tmp >= stor[mid].s && tmp <= stor[mid].e)
            {
                cout << mid + 1 << endl;
                break;
            }
            if(tmp < stor[mid].s) right = mid - 1;
            else left = mid + 1;
        }
    }
}
