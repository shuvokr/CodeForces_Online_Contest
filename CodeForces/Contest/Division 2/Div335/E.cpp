...
struct data
{
    int nod, time, rem;
    data() {}
    data(int a, int b, int c):nod(a), time(b), rem(c) {}
...
void Dijkstra(int s, int e)
{
    ...
    data tmp;
    int nd, tm, rm;
    pq.push(data(s, 0, 0)); // data(pos, time, remain)
    while(!pq.empty())
    {
        tmp = pq.top(); pq.pop();
        nd = tmp.nod; tm = tmp.time; rm = tmp.rem;
        if(cost[ nd ] == tm)
        {
            for(int i = 0; i < graph[ nd ].size(); i++)
            {
                pi pr = graph[ nd ][ i ];
                if(pr.second > sz) continue;
				for(lld j = max(rm + 1, pr.second + 1); j <= sz; i++)
				{
					lld next = (j - rm) * (lld)price[ nd ];
					if(cost[ nd ] + next < cost[ pr.first ])
	                {
	                    cost[ pr.first ] = cost[ nd ] + next;
	                    pq.push(data(pr.first, cost[ pr.first ], j));
	                }
            	}
			}
        }
