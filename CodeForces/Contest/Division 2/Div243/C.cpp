...
void input()
{
    ...
    int ans = -1000000000;
    for(int i = 1; i <= n; i++)
        for(int j = i; j <= n; j++) ans = max(ans, call(i, j));
...
int call(int l, int r)
{
    ...
    mid.clear(); side.clear();

    for(int i = 1; i <= n; i++)
    {
        if(i >= l && i <= r) mid.pb(in[ i ]), ret += in[ i ];
        else side.pb(in[ i ]);
    }
    ...
    for(int i = 0; i < mid.size() && i < side.size() && i < k; i++)
        if(mid[ i ] < side[ i ]) ret = ret - mid[ i ] + side[ i ];
