...
{
    int ci, pi, ind, val;
}team[ 1010 ], res[ 1010 ], table[ 1010 ];

bool cmp(const data &x, const data &y)
{
    if(x.pi > y.pi) return true;
    if(x.pi == y.pi && x.ci < y.ci) return true;
    return false;
}
...
{
    cin >> n;
    for(int i = 1; i <= n; i++) si2(team[i].ci, team[i].pi), team[i].ind = i;
    sort(team + 1, team + 1 + n, cmp);
    cin >> k;
    for(int i = 0; i < k; i++) si1(table[ i ].val), table[i].ind = i + 1;
    sort(table, table + k, cmp1);
    ...
    for(int i = 1; i <= n; i++)
    {
        ...
        for(int j = 0; j < k; j++)
        {
            if(a <= table[ j ].val)
            {
                ans += team[i].pi;
                table[ j ].val = 0;
                res[cou].ind = team[i].ind;
                res[cou].ci = table[j].ind;
                sort(table, table + k, cmp1);
                j = k; cou++;
            }
        }
