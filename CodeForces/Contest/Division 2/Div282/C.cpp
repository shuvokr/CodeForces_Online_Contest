...
void input()
{
    ...
    gets( in );
    bool flag = true;
    int tot = 0, len = strlen( in ), has = 0, x = 0, y = 0, last;
    if(in[0]=='#' || in[0]==')' || in[len - 1] == '(') flag = false;
    int cc = 0, pp = 0;
    for( int i = 0; i < len; i++ ) cc += in[ i ] == '#' ? 1 : 0;

    for(int i = 0; i < len && flag;)
    {
        x = 0; has = 0;
        while((in[ i ] == '(' || in[ i ] == '#') && i < len)
        {
            if(in[ i ] == '(') x++;
            else has++, pp++;
            i++;
        ...
        {
            int k = 0;
            for(int i = 0; i < has; i++) res.push_back( 1 ), k++;
            tot += (x - y - has);
            if(pp == cc)
            {
                res[ res.size() - 1 ] += tot;
