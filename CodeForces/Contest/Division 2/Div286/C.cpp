...
int DP(int pos, int dis);

int n, d, mamo[ 30001 ][ 500 ], offset, mp[ 30010 ] = {0};
...
int DP(int pos, int dis)
{
    ...
    int &ret = mamo[ pos ][ dis + offset ];
    if(ret != -1) return ret;

    int a = 0, b = 0, c = 0;

    a = DP(pos + dis, dis);
    b = DP(pos + dis - 1, dis - 1);
    c = DP(pos + dis + 1, dis + 1);
    return ret = mp[ pos ] + max(a, max(b, c));
}
