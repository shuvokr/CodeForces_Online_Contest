...
lld DP(int mask, int last);

int n, m;
lld in[ 20 ], k, rule[ 20 ], mamo[1 << 18][ 19 ];
map <pi, lld> mp;
...
lld DP(int mask, int last)
{
	//cout << last << endl;
	int cou = 0;
	for(int i = 0; i < n; i++) cou += check(mask, i) ? 1 : 0;
	//cout << cou << endl;
	if(cou >= m) return 0;
	if((1 << n) - 1 == mask) return 0;
	lld &ret = mamo[ mask ][ last ];
	if(ret != -1) return ret;
	ret = 0;
	for(int i = 0; i < n; i++)
	{
		if(check(mask, i)) continue;
		pi p; p.first = last; p.second = i + 1;
		int submask = biton(mask, i);
		//cout << submask << " " << i << " ";
		ret = max(ret, mp[ p ] + in[ i ] + DP(submask, i + 1));
	}
