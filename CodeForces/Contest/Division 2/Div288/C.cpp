...
void input()
{
    ...
    {
        sf("%d", &in);
        if(color[ in ] == r) continue;
        int space = in - last;
        if(space < r - color[ in + 301 ])
        {
            mark = true;
            break;
        }

        int las = in - (r - color[ in + 301 ]) + 1;

        if(color[ in + 301 ] > r || las <= last)
        {
            mark = true;
            break;
        }
        ...
        for(int j = color[ in + 301 ]; j < r; j++)
        {
            ...
            res++;
            int xx = tmp; tmp++;
            for(int k = xx; k < xx + t; k++) color[ k + 301 ]++;
        }
        if(color[ in + 301 ] != r)
        {
            mark = true;
