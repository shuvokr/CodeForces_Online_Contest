...
lld sum, n, k, x, od, ev, in, mamo[ 200005 ][ 3 ];
...
void input()
{
    ...
    CLR(mamo, -1);
    puts(DP(x, 0, ev, od) ? "Stannis" : "Daenerys");
}
int DP(int a, int b, int x, int y)
{
    if(a == 0)
    ...
    lld &ret = mamo[ a ][ b ];
    if(mamo[ a ][ b ] != -1) return mamo[ a ][ b ];
    ret = 0;

    lld s = 0; if(x - 1 >= 0) s = DP(a - 1, 1, x - 1, y);
    lld ss = 0; if(y - 1 >= 0) ss = DP(a - 1, 2, x, y - 1);
