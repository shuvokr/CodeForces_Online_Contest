...
{
    cin >> n;
    for(int i = 0; i < n; i++) {cin >> in[ i ]; tmp[ i ] = in[ i ];}
    for(int i = 0; i < n; i++) sf("%s", bin[ i ]);
    ...
    for(lld i = 0; i < n; i++)
    {
        if(mark[ i ])
        {
            ...
            for(int i = 0; i < k; i++) in[ save[ i ] ] = tmp[ i ];
            //puts("****");
        }
    }
    for(int i = 0; i < n; i++)
...
void DFS(lld s)
{
    ...
    for(lld i = 0; i < n; i++)
    {
        if(s != i && mark[ i ] && (bin[ s ][ i ] == '1' || bin[ i ][ s ] == '1'))
        {
            save.pb( i );
            mark[ i ] = false;
            DFS( i );
