...
{
    in[ 0 ] = 0;
    sf("%I64d %I64d %I64d %I64d", &n, &l, &r, &x);
    for(int i = 1; i <= n; i++) sf("%I64d", &in[ i ]);
    in[ 0 ] = 0;
    graph.assign(20, vi());
    for(int i = 0; i <= n; i++)
    {
        for(int j = i + 1; j <= n; j++) graph[i].pb( j );
    }
...
void DFS(int u, int cou)
{
    ...
    {
        int vv = graph[ u ][ i ];
        if(color[ vv ] == false) continue;
        color[ vv ] = false;
        sum += in[ vv ]; pf("%lld ", sum);
        if(cou > 1 && sum +  >= x && sum >= l && sum <= r) ans++;
        DFS( vv, cou + 1 );
        sum -= in[ vv ];
        color[ vv ] = true;
    }
