...
{
	cin >> n; mp.clear();
	for(int i = 0; i < n; i++) cin >> in[ i ]; t = in[n - 1];
	mp[ in[n - 1] ]++; ans.pb( 0 );
	for(int i = n - 2; i > -1; i--) 
	{
		t = max(in[ i ], t);
		mp[ in[ i ] ]++;
		if(in[ i ] < t) ans.pb(t - in[ i ] + 1);
		else if(t == in[ i ] && mp[ t ] > 1) ans.pb(1);
		else if(t == in[ i ] && mp[ t ] == 1) ans.pb( 0 );
	}
	reverse(ans.begin(), ans.end());
	cout << ans[ 0 ];
	for(int i = 1; i < ans.size(); i++) cout << " " << ans[ i ]; puts("");
}
