...
double DP(int ps, int wst, int last, int mx);

int n;
double p, mamo[ 510 ][ 510 ][ 2 ];
bool mark[ 510 ][ 510 ][ 2 ];
...
void Input()
{
	...
	{
		CLR(mark, false);
		double ans = DP(0, 0, 0, 0);
		pf("%.6lf\n", ans);	
	}
}
double DP(int ps, int wst, int last, int mx)
{
	if(ps == n) return (double)wst;
	double &ret = mamo[ ps ][ wst ][ last ];
	if(mark[ ps ][ wst ]) return ret;
	mark[ ps ][ wst ][ last ] = true;
	int tmp = 1;
	if(last == 1) tmp = wst + 1;
	ret = p * DP(ps + 1, tmp, 1, max(tmp, wst)) + (1.0 - p) * DP(ps + 1, tmp, 0, max(tmp, wst));
	return ret;
}
