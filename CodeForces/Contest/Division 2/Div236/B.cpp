...
struct data
{
    char signn;
    int ind, val;
}ans11[ 1010 ], ans22[ 1010 ];

int n, k, in[ 1010 ], tmp[ 1010 ];
...
{
    si2(n, k);
    for(int i = 1; i <= n; i++) si1(in[ i ]), tmp[ i ] = in[ i ];
    int ans1 = 0, ans2 = 0, fl = 0, id = 0;
    ...
    {
        if(tmp[ 1 ] == ii) continue;
        for(int j = 1; j <= n; j++) in[ j ] = tmp[ j ];
        id = ans1 = 0;
        if(ii > in[ 1 ])
        {
            ans11[id].signn = '+';
            ans11[id].ind = 1;
            ans11[id].val = ii - in[ 1 ];
        }
        else
        {
            ans11[id].signn = '-';
            ans11[id].ind = 1;
            ans11[id].val = in[ 1 ] - ii;
        }
        in[ 1 ] = ii; id++; ans1++;
        for(int i = 2; i <= n; i++)
        {
        ...
        {
            ans2 = ans1;
            for(int i = 0; i < ans1; i++) ans22[ i ] = ans11[ i ];
        }
    }

    pi1(ans2);
    for(int i = 0; i < ans2; i++) pf("%c %d %d\n", ans22[i].signn, ans22[i].ind, ans22[i].val);
}
