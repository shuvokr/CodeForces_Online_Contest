...
long long n, in[ 3000010 ], pin[ 3000010 ], res, remov;
...
void input()
{
    ...
    sort(pin + 1, pin + n + 1);
    for(int i = 1; i <= n; i++) in[ i ] = in[i - 1] + pin[ i ];
    //for(int i = 1; i <= n; i++) cout << in[ i ] << " "; puts("");
    res = remov = 0ll;
    for(int i = 1; i < n; i++)
    {
        res += in[ n ] - in[i - 1] + in[ i ] - remov;
        remov += pin[ i ];
        //cout << i << " " << res << " " << remov << endl;
    }
    cout << res + in[ n ] - in[n - 1] << endl;
}
